...
print('External Validation X Size:', TS_data.shape)
print('External Validation y Size:', TS_out.shape)
...
for train_index, val_index in kf.split(X_data):
    ...
    print('Y Test Shape:', np.array(y_test).shape)
    ...
    x_1 = base_model.output

    x_2 = keras.layers.Conv2D(10, 3, padding='same', activation='relu')(x_1)
    x = keras.layers.Flatten()(x_2)
    ...
    heatmap_model = Model(base_model.input, x_2)
    ...
    model.save('./Models/1.3/{}/Main Model/model.h5'.format(fold_no))
    heatmap_model.save('./Models/1.3/{}/Heatmap Model/model.h5'.format(fold_no))
    ...
    with open('./Results/1.3/{}/classification_report.txt'.format(fold_no), 'w') as f:
    ...
    disp.plot()
    plt.savefig('./Results/1.3/{}/confusion_matrix.png'.format(fold_no), bbox_inches='tight')
    ...
    plt.savefig('./Results/1.3/{}/roc_curve.png'.format(fold_no), bbox_inches='tight')
    ...
    plt.savefig('./Results/1.3/{}/precision_recall_curve.png'.format(fold_no), bbox_inches='tight')
    ...
    output_dir_0 = './Results/1.3/{}/Heatmap/0/'.format(fold_no)
    output_dir_1 = './Results/1.3/{}/Heatmap/1/'.format(fold_no)
    output_dir_2 = './Results/1.3/{}/Heatmap/2/'.format(fold_no)

    os.makedirs(output_dir_0, exist_ok=True)
    os.makedirs(output_dir_1, exist_ok=True)
    os.makedirs(output_dir_2, exist_ok=True)

    def classify_and_save_with_gradcam(model, img, label, id, last_conv_layer_name="conv5_block3_3_conv", classifier_layer_names=["predictions"]):
    ...
    for i in range(len(X_test)):
        classify_and_save_with_gradcam(model, X_test[i], y_test_arg[i], i)
