...
FA_data = np.concatenate((FA_data[:, :384, :384], TS_data), axis=0)
...
X = np.concatenate((FA_data, FG_data[:, :384, :384], FNS_data[:, :384, :384]), axis=0)
...
model.save('./Models/Main Model V1.1')
heatmap_model.save('./Models/Heatmap Model V1.1')
...
for i in range(len(NM)):
    ...
    display.plot()
    plt.savefig('./Results/Results V1.1/ROC (' + NM[i] + ' Vs. The Rest).JPG')
    ...
    display.plot()
    plt.savefig('./Results/Results V1.1/PR (' + NM[i] + ' Vs. The Rest).JPG')